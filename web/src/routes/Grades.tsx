...
export function Grades() {
  ...
  return (
    <AppShell back="/subjects" title={t(`subjects.${subject}`)}>
      ...
      <div className="grid grid-cols-2 content-center gap-5 sm:grid-cols-3">
        {grades.map((g, idx) => (
          <motion.button
            key={g}
          ...
          >
            <span className="font-display text-6xl text-sun">{gradeLetter(g)}</span>
            <span className="text-cream/70">{t("grades.grade", { n: gradeLetter(g) })}</span>
          </motion.button>
        ))}
      </div>
    </AppShell>
