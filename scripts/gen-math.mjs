...
// Emit up to maxSets lessons of setSize questions each from a question pool.
// shuffleQs=false keeps the pool order (used for balanced binary categories).
function mcSets(catKey, grade, diff, title, prompt, pool, { setSize = 4, maxSets = 2, shuffleQs = true } = {}) {
  const sets = chunk(shuffleQs ? shuffle(pool) : pool, setSize).filter((s) => s.length >= 2).slice(0, maxSets);
  sets.forEach((qs, i) => {
    const id = `math-g${grade}-${catKey}-set-${pad(i + 1)}`;
    const lines = base(id, grade, diff, title, "multiple-choice", prompt);
    lines.push("questions:");
    qs.forEach((q) => lines.push(...qLines(q)));
    lines.push(REWARD3);
    write(id, lines);
    questions += qs.length;
  });
...
// ---------- arithmetic / comparison / counting (unchanged shapes) ----------
...
function cmpSet(id, grade, diff, n, max) {
  ...
  const lines = base(id, grade, diff, `מִי גָּדוֹל יוֹתֵר? — סֵט ${Number(id.slice(-2))}`, "comparison",
...
// word problems: [{prompt, correct, distractors}]
function wordSets(grade, diff, list, maxSets = 2) {
  mcSets("word", grade, diff, "בְּעָיוֹת מִלּוּלִיּוֹת", "פִּתְרוּ אֶת הַבְּעָיוֹת",
    list.map((w) => ({ prompt: w.p, promptText: w.p, correct: w.a, distractors: w.d })), { setSize: 4, maxSets });
}
...
mcSets("even", 1, 2, ...T_EVEN, evenOddPool(1, 10), { maxSets: 2, shuffleQs: false });
mcSets("succ", 1, 1, ...T_SUCC, succPool(1, 19), { maxSets: 2 });
mcSets("pred", 1, 1, ...T_PRED, predPool(2, 20), { maxSets: 2 });
mcSets("seq", 1, 2, ...T_SEQ, seqPool([2, 5, 10], [1, 2, 3, 4], 60), { maxSets: 2 });
mcSets("num", 1, 1, ...T_NUM, numRecPool(1, 12), { maxSets: 2 });
mcSets("clock", 1, 2, ...T_CLOCK, clockPool(range(1, 12)), { maxSets: 2 });
...
// ===================== GRADE 2 =====================
mcSets("place-tens", 2, 2, "כַּמָּה עֲשָׂרוֹת?", "כַּמָּה עֲשָׂרוֹת יֵשׁ בַּמִּסְפָּר?", tensPool([47, 63, 28, 91, 35, 72, 56, 84]), { maxSets: 2 });
mcSets("place-units", 2, 2, "כַּמָּה יְחִידוֹת?", "כַּמָּה יְחִידוֹת יֵשׁ בַּמִּסְפָּר?", unitsPool([47, 63, 28, 91, 35, 72, 56, 84]), { maxSets: 1 });
mcSets("even", 2, 2, ...T_EVEN, evenOddPool(10, 30), { maxSets: 2, shuffleQs: false });
mcSets("seq", 2, 2, ...T_SEQ, seqPool([10, 5, 3], [10, 20, 25, 12], 120), { maxSets: 2 });
mcSets("sign", 2, 2, "אֵיזֶה סִימָן מַתְאִים?", "אֵיזֶה סִימָן מַתְאִים בֵּין הַמִּסְפָּרִים?", signPool([[4, 7], [9, 3], [6, 6], [12, 8], [5, 11], [10, 10], [15, 9], [7, 13]]), { maxSets: 2 });
mcSets("half", 2, 2, "חֲצִי", "כַּמָּה זֶה חֲצִי?", fracPool([4, 6, 8, 10, 12, 14, 16, 20], 2, "½"), { maxSets: 2 });
...
// ===================== GRADE 3 =====================
mcSets("ops", 3, 3, ...T_OPS, opsPool([[2, 3, 4, 0], [10, 2, 3, 2], [3, 2, 4, 1], [4, 2, 3, 3], [5, 3, 2, 0], [12, 2, 3, 2], [2, 4, 3, 1], [6, 1, 5, 3]]), { maxSets: 2 });
mcSets("seq", 3, 2, ...T_SEQ, seqPool([3, 4, 6, 7], [3, 4, 6, 7], 100), { maxSets: 2 });
mcSets("round-tens", 3, 3, "עִגּוּל לַעֲשָׂרוֹת", "עַגְּלוּ אֶת הַמִּסְפָּר לָעֲשָׂרוֹת", roundPool([47, 83, 28, 61, 35, 72, 56, 94], 10), { maxSets: 2 });
mcSets("half", 3, 2, "חֲצִי", "כַּמָּה זֶה חֲצִי?", fracPool([10, 16, 20, 24, 30, 40, 50, 100], 2, "½"), { maxSets: 1 });
mcSets("quarter", 3, 2, "רֶבַע", "כַּמָּה זֶה רֶבַע?", fracPool([8, 12, 16, 20, 24, 40], 4, "¼"), { maxSets: 1 });
mcSets("clock", 3, 2, ...T_CLOCK, clockPool(range(1, 12)), { maxSets: 1 });
...
// ===================== GRADE 4 =====================
mcSets("ops", 4, 3, ...T_OPS, opsPool([[4, 2, 3, 3], [20, 3, 4, 2], [5, 4, 2, 1], [2, 5, 3, 0], [6, 2, 4, 3], [30, 5, 2, 2], [3, 3, 3, 1], [8, 2, 5, 0]]), { maxSets: 2 });
mcSets("round-hundreds", 4, 3, "עִגּוּל לַמֵּאוֹת", "עַגְּלוּ אֶת הַמִּסְפָּר לַמֵּאוֹת", roundPool([278, 412, 650, 189, 530, 333, 720, 845], 100), { maxSets: 2 });
mcSets("quarter", 4, 3, "רֶבַע", "כַּמָּה זֶה רֶבַע?", fracPool([12, 16, 20, 24, 40, 100], 4, "¼"), { maxSets: 1 });
mcSets("third", 4, 3, "שְׁלִישׁ", "כַּמָּה זֶה שְׁלִישׁ?", fracPool([9, 12, 15, 18, 30, 60], 3, "⅓"), { maxSets: 1 });
...
// ===================== GRADE 5 =====================
mcSets("ops", 5, 3, ...T_OPS, opsPool([[20, 3, 4, 2], [4, 5, 2, 1], [6, 2, 4, 0], [30, 4, 5, 2], [5, 5, 3, 3], [10, 2, 6, 2], [3, 6, 4, 1], [8, 3, 2, 0]]), { maxSets: 2 });
mcSets("half", 5, 3, "חֲצִי", "כַּמָּה זֶה חֲצִי?", fracPool([24, 30, 50, 80, 100, 120], 2, "½"), { maxSets: 1 });
mcSets("frac-cmp", 5, 3, "הַשְׁווּ שְׁבָרִים", "אֵיזֶה שֶׁבֶר גָּדוֹל יוֹתֵר?", [
...
mcSets("dec", 5, 3, "שְׁבָרִים עֶשְׂרוֹנִיִּים", "כַּמָּה זֶה?", [
...
// ===================== GRADE 6 =====================
mcSets("ops", 6, 3, ...T_OPS, opsPool([[6, 12, 3, 1], [20, 4, 3, 2], [5, 6, 4, 0], [40, 5, 6, 2], [4, 7, 3, 3], [10, 3, 5, 0], [8, 4, 2, 1], [30, 2, 7, 2]]), { maxSets: 2 });
mcSets("frac", 6, 3, "חִבּוּר שְׁבָרִים", "כַּמָּה זֶה?", [
...
mcSets("frac-cmp", 6, 3, "הַשְׁווּ שְׁבָרִים", "אֵיזֶה שֶׁבֶר גָּדוֹל יוֹתֵר?", [
...
mcSets("dec", 6, 3, "כֶּפֶל עֶשְׂרוֹנִי", "כַּמָּה זֶה?", [
...
mcSets("unknown", 6, 3, "מַהוּ הַנֶּעְלָם?", "אֵיזֶה מִסְפָּר מַשְׁלִים אֶת הַתַּרְגִּיל?", [
